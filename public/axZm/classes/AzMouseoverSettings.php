...
if (!class_exists('AzMouseoverSettings', false)) {
    ...
    {
        public $vendor = '';
        public $last_updated = '2021-04-20';
        public $config = array();
        public $items = array();
...
<ul>
    ...
    <li>"title" - optional title</li>
                ...
                'firstItemToLoad' => array(
                    ...
                    'comment' => array(
                        'EN' => '
<p>With this option, you can define which item in the gallery should load first.
    You do not need to define it.
    It is only useful if you want to trigger this plugin as a modal window or in an APP container from a different gallery.
    Otherwise, you have the "firstTypeToLoad" option where you can set to load an image, a 360-view, or a video first.
    With the "firstTypeToLoad" option, however, you cannot specify the exact item that you want to load initially.
    With this "firstItemToLoad" option, you can!
</p>
<p>A typical internal order array may look like this, and the "image", "360", and "video" keys indicate the type always starting with those prefixes.
</p>
...
<p>You have two possibilities to indicate what you want to load first.
</p>
<p><b>As a string</b>, you can set the "firstItemToLoad" option as, for example, "360_1" to load the first 360-view,
    "image_4" to load the fourth image, or "video_1" to initialize the gallery with the first video loaded into it.
    That is regardless of the position of, e.g., "image_4" in the array because all values are unique.
    There can be no second "image_4" in that array.
</p>

<p><b>As an integer</b>, which is the index number of the array.
    Indexes in an array start from 0 and not 1.
    Thus, taken this array
<pre class="language-js"><code class="language-js">
    // 360 views, images, and videos.
   ["360_1", "360_2", "image_1", "image_2", "image_3", "image_4", "image_5", "image_6", "video_1", "video_2"]
</code></pre>
...
Die Schaltfläche erscheint nur im gezoomten Zustand.'
                ...
                'firstTypeToLoad' => array(
                    ...
                    'options' => array(
                        array('images', 'Images'),
                        array('images360', '360/3D-views'),
                        array('videos', 'Videos'),
                    ),
                    'comment' => array(
                        'EN' => '
Select which type of content should be loaded on initialization if such type is present.
For example, you can choose to load a 360-view regardless of its position in the gallery first.
If you are integrating this extension manually and initialize in an APP box or modal window,
use the "firstItemToLoad" option to define exactly which image, 360-view, or video you want to load first.
Possible values are: "images", "images360", and "videos".',
                        'DE' => '
Wählen Sie aus, welcher Inhaltstyp bei der Initialisierung geladen werden soll, wenn ein solcher Typ vorhanden ist.
Beispielsweise können Sie eine 360-Ansicht unabhängig von ihrer Position in der Galerie zuerst laden.
Wenn Sie diese Erweiterung manuell integrieren und in einer APP-Box oder einem modalen Fenster initialisieren,
verwenden Sie die Option "firstItemToLoad", um genau zu definieren, welches Bild, welche 360-Ansicht
oder welches Video Sie zuerst laden möchten.
Mögliche Werte sind: "images", "images360" und "videos".',
                    ),
                ),
...
On default, when not defined / empty, the language is browsers or systems language.',
                        'DE' => '
