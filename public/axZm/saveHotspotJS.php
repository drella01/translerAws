...
if (empty($pass) && !empty($_SERVER['AJAXZOOM_PASS_HOTSPOTJSON'])) {
	$pass = $_SERVER['AJAXZOOM_PASS_HOTSPOTJSON'];
}
...
if ($allowSave == false) {
    echo '
	> Because of security reasons, you can not create or save hotspot settings to a JavaScript or JSON file.<br>
	> When using the AJAX-ZOOM PHP version, open the "/axZm/saveHotspotJS.php" file and set $pass variable to your password.<br>
	> Alternatively, define the AJAXZOOM_PASS_HOTSPOTJSON environment variable.<br>
	';
    exit;
}
...
if (!(isset($_POST['keepFormat']) && $_POST['keepFormat'] == '1')) {
    ...
    $_POST['jsCode'] = preg_replace('!\s+!', ' ', $_POST['jsCode']);
    $_POST['jsCode'] = str_replace('{ ', '{', $_POST['jsCode']);
    $_POST['jsCode'] = str_replace(' }', '}', $_POST['jsCode']);
}
