...
class axZmH
{
        ...
        if (isset($zoom['config']['allowAjaxQuery']) && $zoom['config']['allowAjaxQuery']['images']) {
            if (!empty($zoom['config']['pic_list_data']) && $this->allowAjaxQueryCheck($zoom)) {
                ...
                foreach($zoom['config']['pic_list_data'] as $k => $v) {
                    $returnData[$k]['fileName'] = isset($v['fileName']) ? $v['fileName'] : '';
                    $returnData[$k]['thumbDescr'] = isset($v['thumbDescr']) ? $v['thumbDescr'] : '';
                    $returnData[$k]['fullDescr'] = isset($v['fullDescr']) ? $v['fullDescr'] : '';
                }
    ...
    public function getDataImages($zoom, $JSONencode = true)
    {
        ...
        foreach($zoom['config']['pic_list_data'] as $k => $v) {
            ...
            $returnData[$n]['thumbDescr'] = isset($v['thumbDescr']) ? $v['thumbDescr'] : '';
            $returnData[$n]['fullDescr'] = isset($v['fullDescr']) ? $v['thumbDescr'] : '';
        }
        if (!empty($returnData)) {
            $return[0] = 'ok';
            $return[1] = $returnData;
    ...
    public function checkConfig($zoom)
    {
        ...
        if ($zoom['config']['galHorHeight'] == 'auto') {
            ...
            + (int)$zoom['config']['galHorPadding']['top']
            + (int)$zoom['config']['galHorPadding']['bottom']
            + (int)$zoom['config']['galHorOpt']['thumbLiStyle']['height']
        ...
        if (is_array($_GET['zoomData'])) {
            ...
            foreach($_GET['zoomData'] as $k => $v) {
                if ( (isset($_GET['zoomLoadAjax']) || isset($_GET['loadZoomAjaxSet']) || isset($_GET['setHW'])) && !(isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER']) && substr($v['p'],0, 3) == '../') {
                    echo "
<div style='padding: 10px; font-size: 150%; background-color: #CC1100; color: #FFFFFF' class=''>
                ...
                if (!isset($_GET['str'])) {
                    ...
                    if (!isset($_GET['setHW']))
                    {
                        ...
                        foreach ($pic_list_data as $k => $v) {
                            ...
                            $zoom['config']['galArray'][$k]['ow'] = isset($v['imgSize'][0]) ? $v['imgSize'][0] : '';
                            $zoom['config']['galArray'][$k]['oh'] = isset($v['imgSize'][1]) ? $v['imgSize'][1] : '';
                            $zoom['config']['galArray'][$k]['iw'] = isset($v['thumbSize'][0]) ? $v['thumbSize'][0] : '';
                            $zoom['config']['galArray'][$k]['ih'] = isset($v['thumbSize'][1]) ? $v['thumbSize'][1] : '';
                            $zoom['config']['galArray'][$k]['tD'] = isset($v['thumbDescr']) ? $v['thumbDescr'] : '';
                            $zoom['config']['galArray'][$k]['fD'] = isset($v['fullDescr']) ? $v['fullDescr'] : '';
            ...
            if (is_dir($zoom['config']['thumbDir'])) {
                ...
                if (!empty($globResult)) {
                    foreach ($globResult as $file) {
                        $fileName = $this->getf('_', $this->getf('.', $this->getl('/', $this->checkSlash($file, 'remove'))));
                        if ($fileName == $picName) {
                            unlink($file);
                        }
                    }
