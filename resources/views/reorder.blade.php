...
<div class="container">
    ...
    <div class="mt-4">
        ...
        <ul class="row" id="photoble" style="list-style-type: none;">
            @foreach ($photos as $key=>$photo)
                <li class="ui-state-default" style="display: inline" id={{ $photo->id }}>
                    <form action="{{ route('photos.destroy', $photo) }}" method="POST">
                        ...
                        <div class="p-3">
                            <input type="checkbox" class="custom" value={{$photo->url}} name='testing[]'>
                            <input type="hidden" name="test[]" value={{$key}}>
                        </div>
            ...
            @endforeach
                        ...
                        <div class="m-4 form-inline">
                            <button class="btn btn-info" formaction="{{ route('photos.destroyselection',$vehicle) }}">Eliminar seleccion</button>
                            <button class="btn btn-danger ml-4" formaction="{{ route('photos.destroyall',$vehicle) }}">Eliminar todas</button>
                        </div>
                    </form>
        ...
        <div class="m-4 form-inline">
            ...
            <a class="btn btn-success ml-4 mr-4" href="{{ route('vehicles.show',$vehicle) }}">Volver</a>
        </div>
    </div>
</div>
